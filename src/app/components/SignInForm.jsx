...
const SignInForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (result.success) {
      if (isSignup) {
        setLocalError('Registration successful! Please check your email to verify your account before logging in.');
        setTimeout(() => router.push('/login'), 3000);
      } else {
        router.push('/chat');
      }
    } else {
      setLocalError(result.error || 'An unexpected error occurred.');
    }
  };
        ...
        <form onSubmit={handleSubmit} className="bg-white p-2 w-full">
          ...
          {!isSignup && (
            <div className="flex items-center justify-between mb-5">
              ...
              <Link href="/forgot-password" className="text-red-600 text-base md:text-lg">
                Forgot password?
              </Link>
            </div>
          )}
          ...
          {localError && (
            <p className="text-red-500 text-sm text-center mb-4">{localError}</p>
          )}
