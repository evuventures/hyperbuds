...
const SignUpForm = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (!form.firstName.trim() || !form.lastName.trim() || !form.username.trim() || !form.email.trim() || !form.phone.trim() || !form.password.trim() || !form.confirmPassword.trim()) {
        ...
        if (result.success) {
            setLocalSuccess(result.message || 'Registration successful! Please check your email to verify your account.');
            setForm({
                firstName: '', lastName: '', username: '', email: '', phone: '',
                password: '', confirmPassword: '', remindMe: false,
            });
            setTimeout(() => router.push('/login'), 3000);
        } else {
            setLocalError(result.error || 'An unexpected error occurred during registration.');
        }
    }
        ...
        <div className="relative group">
            ...
            <div className="relative">
                <input
                    ...
                    disabled={disabled}
                />
                {Icon && <Icon className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-500 transition-colors duration-300" />}
                {showToggle && (
                    <button
                        type="button"
                ...
                <div className="w-full lg:w-1/2 flex items-center justify-center p-8">
                    <div className="w-full max-w-md">
                        ...
                        <div className="flex gap-3 mb-8">
                            <button type="button" className="flex-1 flex items-center justify-center gap-3 h-12 bg-white/60 backdrop-blur-sm border border-white/30 rounded-xl hover:bg-white/80 transition-colors duration-300 group">
                        ...
                        <form onSubmit={handleSubmit} className="space-y-6">
                            ...
                            <div className="flex items-center justify-between">
                                ...
                                <Link href="/forgot-password" className="text-sm text-purple-600 hover:text-purple-500 transition-colors">
                                    Forgot password?
                                </Link>
                            </div>
                            ...
                            {localSuccess && (
                                <p className="text-green-600 text-sm text-center">{localSuccess}</p>
                            )}

                            <button
                                type="submit"
                                // onClick={handleSubmit} // onClick is handled by form onSubmit
